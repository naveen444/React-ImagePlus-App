...
function App() {
	...
	return (
		<div className={`container-fluid p-2 ${background? 'bg-dark' : 'bg-light'}`}>
			<div className='p-2'>
				<div className="header-bar row m-0 my-2 my-md-4 px-md-5">
					<div className='logo-wrapper col col-auto m-0 me-3 p-0'>
					...
					<div className="search-bar col input-group m-0 me-3 p-0">
						<input type="text" className="form-control m-0 shadow" placeholder='Try "Dog" or "Boat"' onChange={searchText} value={search} />
					</div>
					<div className='search-btn col col-1 m-0 me-3 p-0'>
						<button className="btn btn-primary shadow w-100" onClick={searchThis}><GoSearch size={20} /></button>
					</div>
					<div className="get-random-btn col col-2 p-0 m-0 me-3" style={{width: '12%'}}>
						<button className='btn btn-primary shadow w-100' onClick={getImage}>Get Random</button>
					</div>
					<div className="mode-btn col col-auto p-0 m-0 me-3">
				...
				<div className={`filters row m-0 my-4 mb-0 px-md-5 justify-content-end ${showFilters? 'show' : ''}`}>
